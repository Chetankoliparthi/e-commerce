...
const PlaceOrder = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      for (const items in cartitems) {
        console.log("Processing item:", items); // Debug log
  
        for (const item in cartitems[items]) {
          console.log(`Checking size ${item} with quantity ${cartitems[items][item]}`); // Debug log
  
          if (cartitems[items][item] > 0) {
            const itemInfo = structuredClone(products.find(product => product._id === items));
  
            if (itemInfo) {
              console.log("Found product info:", itemInfo); // Debug log
              itemInfo.size = item;
              itemInfo.quantity = cartitems[items][item];
              orderItems.push(itemInfo);
            } else {
              console.warn(`Product with ID ${items} not found in products`); // Debug log
            }
          }
        }
      }
  ...
  <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
    ...
    <div className='flex flex-col gap-4 w-full sm:max-w-[480px]'>
      ...
      <input required onChange={onChangeHandler} name='email' value={formData.email} type="email" placeholder='Enter the e-mail address' className='border border-gray-300 rounded py-1.5 px-3.5 w-full' />
